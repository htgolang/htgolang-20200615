...
func alarm() {
	...
	go func() {
		for now := range time.Tick(time.Minute) {
			...
			models.DefaultAlarmManager.CreateAlarm(1, uuid, fmt.Sprintf("终端离线时间超过了%d分钟", alarm.Time))
		}
	}()
	...
	go func() {
		for now := range time.Tick(time.Minute) {
			...
			var uuid = []orm.Params{}
			orm.NewOrm().Raw("SELECT uuid FROM resource WHERE deleted_time IS NULL AND created_time >= ? AND cpu_percent >= ? GROUP by uuid having count(*) >= ?", startTime, cpuThreshold, cpuCounter).Values(&uuid)
			models.DefaultAlarmManager.CreateAlarm(2, uuid, fmt.Sprintf("CPU使用率在%d分钟以内,连续%d次超过%d%", alarm.Time, alarm.Counter, alarm.Threshold))
		}
	}()
	...
	go func() {
		for now := range time.Tick(time.Minute) {
			...
			startTime := now.Add(-time.Duration(windowTime) * time.Minute)
			var uuid = []orm.Params{}
			orm.NewOrm().Raw("SELECT uuid FROM resource WHERE deleted_time IS NULL AND created_time >= ? AND ram_percent >= ? GROUP by uuid having count(*) >= ?", startTime, ramThreshold, ramCounter).Values(&uuid)
