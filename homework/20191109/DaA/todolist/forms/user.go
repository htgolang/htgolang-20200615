...
func (f *UserCreateForm) Valid(v *validation.Validation) {
	...
	v.MaxSize(f.Name,2,"name.name").Message("用户名长度必须在%d到%d之间",2,16)
	...
	v.Range(f.Sex,0,1,"sex.sex").Message("性别选择错误")
	...
	v.MaxSize(f.Desc,128,"desc.desc").Message("备注长度必须在%d之间",128)
}
...
type ModifyPasswordForm struct {
	OldPassword string	`form:"oldPassword,passowrd,旧密码"`
	NewPassword string	`form:"newPassword,password,新密码"`
	PasswordVerify string	`form:"passwordVerify,password,再次输入新密码"`
