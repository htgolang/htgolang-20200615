...
func (c *UserCreateForm) Valid(v *validation.Validation) {
	...
	v.Range(c.Sex, 0, 1, "sex.sex").Message("性别选择不正确")
	...
	v.MaxSize(c.Desc, 128, "desc.desc").Message("备注长度必须在%d之内", 128)
}
...
func (c *ModifyPasswordForm) Valid(v *validation.Validation) {
	...
	if !c.User.ValidatePassword(c.OldPassword) {
		_ = v.SetError("oldPassword", "密码错误")
	}

	// 使用beego validation提供的验证器验证最小和最大长度
	v.MinSize(c.NewPassword, 6, "newPassword.newPassword").Message("密码最小长度位%d位", 6)
