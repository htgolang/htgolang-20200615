...
func UserAction(w http.ResponseWriter, r *http.Request) {
	...
	q := strings.TrimSpace(r.FormValue("q")) //  strings.TrimSpace 去掉前后的空格
	fmt.Println(q)
	...
	tpl.Execute(w, struct { // 这里用匿名函数将两个值传递 【匿名结构体的模板应用】
		Q     string // 这里的Q与模板的Q对应
...
func UserActionModify(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		models.ModifyUser(id, name, password, birthday, tel, addr, desc)
		http.Redirect(w, r, "/users/list/", http.StatusFound)
	}
