...
func main() {
	...
	secNum := 0
	for _, value := range ints {
		if value > secNum && value != maxNum {
			secNum = value
		}
	}
	fmt.Printf("第二大的数 : %d\n", secNum)

	// 4. 获取映射中所有key组成的切片; 5. 获取映射中所有value组成的切片
	mapping := map[string]int{"age": 18, "height": 180, "weight": 60}
	fmt.Printf("数组的内容为：%v\n", mapping)
	...
	fmt.Printf("所有Key的切片: %v\n", keys)
	fmt.Printf("所有Value的切片: %v\n", values)

	// 6. 我有一个梦想 每个字符出现次数 rune => int, count => []rune a = 2 b = 3 c = 2
	//
	//2 => ['a', 'c'] 3 => ['b']
