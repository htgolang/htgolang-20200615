...
func (c *TenantCloud) GetInstance() []*cloud.Instance {
	...
	for offset < total {
		...
		total, instances := c.GetInstancesByOffsetLimit(offset, limit)
		if offset == 0 {
...
func (c *TenantCloud) GetInstancesByOffsetLimit(offset , limit int64) (int64, []*cloud.Instance){
	client, err := cvm.NewClient(c.credential, c.region, c.profile)
	if err != nil {
		//日志
		return 0,nil
	...
	for index, instance := range instances {
		...
		rt[index] = &cloud.Instance{
			...
			Status:       c.trnasformsStatus(*instance.InstanceState),
...
func (c *TenantCloud) trnasformsStatus(status string) string {
