...
func (c *VirtualMachineController) Start(){
	...
	if vm := models.DefaultVirtualMachineManager.GetById(id); vm != nil {
		if sdk, ok := cloud.DefaultManager.Cloud(vm.Platform.Type); ok {
			...
			if err := sdk.StartInstance(vm.UUID); err == nil {
				json["code"], json["text"], json["result"] = 200,"开机成功",nil
			} else {
				json["code"], json["text"], json["result"] = 403,"操作开机失败",nil
			}
		}
	}
	c.Data["json"] = json
	...
	if vm := models.DefaultVirtualMachineManager.GetById(id); vm != nil {
		if sdk, ok := cloud.DefaultManager.Cloud(vm.Platform.Type); ok {
			...
			if err := sdk.StopInstance(vm.UUID); err ==  nil {
				json["code"], json["text"], json["result"] = 200,"关机成功",nil
			} else {
				json["code"], json["text"], json["result"] = 403,"操作关机失败",nil
			}
		}
	}
	c.Data["json"] = json
...
func (c *VirtualMachineController) Restart(){
	...
	if vm := models.DefaultVirtualMachineManager.GetById(id); vm != nil {
		if sdk, ok := cloud.DefaultManager.Cloud(vm.Platform.Type); ok {
			sdk.Init(vm.Platform.Addr, vm.Platform.Region, vm.Platform.AccessKey, vm.Platform.SecrectKey)
			if err := sdk.RestartInstance(vm.UUID); err ==  nil {
				json["code"], json["text"], json["result"] = 200,"重启成功",nil
			} else {
				json["code"], json["text"], json["result"] = 403,"操作重启失败",nil
			}
		}
	}
	c.Data["json"] = json
