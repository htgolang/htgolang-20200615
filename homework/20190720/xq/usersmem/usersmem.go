...
func gobdecode(file string) (map[int]User, error) {
	...
	if tmpfile {
		...
		if err == nil{
			...
			csvcoder := csv.NewReader(f)
			i := 0
			for {
				line, err := csvcoder.Read()
				if err != nil {
					if err != io.EOF {
						fmt.Println(err)
					}
...
func goencode(file string, users map[int]User) {
	...
	//tmpfile, err := os.OpenFile(file,os.O_APPEND|os.O_CREATE|os.O_WRONLY, os.ModePerm)

	//
	//tmpfile, err := os.Create(file)
	//if err == nil {
	//	defer tmpfile.Close()
	//	csvencode := csv.NewWriter(tmpfile)
	//
	...
	if tmp {
		...
		if err == nil {
			...
			for _, user := range users {
				data :=[]string{strconv.Itoa(user.ID), user.Name, user.Birthday.Format("2006/01/02"), user.tel, user.addr, user.desc}
				csvencode.Write(data)
			}
