...
// 反序列化
func csvdecode(file string) (map[int]User, error) {
	users := map[int]User{}
	_, err := os.Stat(file)
	if err != nil {
		if os.IsNotExist(err) {
	...
	if err == nil {
		defer f.Close()
		csvdecode := csv.NewReader(f)
		i := 0
		for {
			line, err := csvdecode.Read()
			if err != nil {
				if err != io.EOF {
					fmt.Println(err)
				}
				...
				users[id] = User{id, line[1], birthday, line[3], line[4], line[5]}
			}
		}
		return users, nil
	} else {
		return nil, fmt.Errorf("csvdecode 打开文件失败: %s", err)
	}

}

// 序列化
func csvencode(file string, users map[int]User) {
	...
	if err != nil {
		...
		defer infofile.Close()
		csvencode := csv.NewWriter(infofile)
		csvencode.Write([]string{"ID", "Name", "Birthday", "Tel", "Addr", "Desc"})
		for _, user := range users {
			csvencode.Write([]string{strconv.Itoa(user.Id), user.Name, user.Birthday.Format("2006/01/02"), user.Tel, user.Addr, user.Desc})
		}
		csvencode.Flush()
	}
}
...
func getUserID() (id int) {
	users, err := csvdecode(getUserfile())
	if err != nil {
		fmt.Println("请检查csvdecode函数.")
	}
...
func Add() {
	...
	if err := validUser(user); err == nil {
		users[id] = user
		// 序列化
		csvencode(filepath.Join(info, strconv.FormatInt(time.Now().UnixNano(), 10)), users)
		fmt.Println("[+]添加成功")
	} else {
		fmt.Print("[-]添加失败:")
		fmt.Println(err)
