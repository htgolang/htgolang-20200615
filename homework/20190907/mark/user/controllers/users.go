...
func UserActionModify(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		addr := r.PostFormValue("addr")
		desc := r.PostFormValue("desc")
		fmt.Println("MethodPostdesc---------:", id, name, password, birthday, sex, tel, addr, desc)
	...
	//fmt.Println("GET-FormValue-ID:", id)
	user, err := models.GetUserByID(id)
	//fmt.Println("GetUserByID：", err, user.Name)
	if err != nil {
		fmt.Println("GetUserByIDErr：", err)
		http.Redirect(w, r, "/users/list/", http.StatusFound)
	} else {
		tpl := template.Must(template.New("modifyuser.html").ParseFiles("views/user/modifyuser.html"))
		//fmt.Println("template.New:", w, user)
		tpl.Execute(w, user)
	}
